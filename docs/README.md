...
1. **后端 API**：默认监听 `0.0.0.0:8000`，局域网内其他设备可通过 `http://<服务器IP>:8000` 访问
2. **前端 Web UI**：默认监听 `0.0.0.0:5173`，局域网内其他设备可通过 `http://<服务器IP>:5173` 访问
3. **CORS 配置**：已允许所有来源跨域访问，支持局域网内设备调用 API
...
- 局域网访问：`http://192.168.x.x:5173`（替换为实际服务器IP）
